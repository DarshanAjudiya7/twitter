        ...
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="grid gap-4">
              ...
              <div className="space-y-2">
                {channels.slice(0, 4).map((channel) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-zinc-500">{channel.unread} unread</span>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <article className="overflow-hidden rounded-lg border border-white/10 bg-zinc-950/80">
              <img src={featuredPost.cover} alt="" className="h-44 w-full object-cover" />
              <div className="p-4">
                ...
                <p className="mt-3 text-sm leading-6 text-zinc-400">{featuredPost.excerpt}</p>
              </div>
            </article>
          </div>
        </div>
      ...
      <section className="px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[260px_minmax(0,1fr)_320px]">
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-lg border border-white/10 bg-zinc-950/80">
              <div className="flex items-center justify-between border-b border-white/10 p-4">
                <div>
                  <p className="text-xs uppercase tracking-wide text-zinc-500">Trending discussions</p>
                  <h2 className="text-lg font-semibold text-white">Questions developers are answering now</h2>
                </div>
              ...
              <div className="divide-y divide-white/10">
                {discussions.map((discussion) => (
                  <Link href={`/chat/${discussion.channel}`} key={discussion.title} className="block p-4 transition hover:bg-white/[0.03]">
                    ...
                    <h3 className="mt-2 font-semibold text-zinc-100">{discussion.title}</h3>
                    <div className="mt-3 flex gap-4 text-sm text-zinc-500">
                      <span>{discussion.votes} upvotes</span>
          ...
          <aside className="space-y-4 lg:sticky lg:top-24 lg:h-fit">
            <div className="rounded-lg border border-white/10 bg-zinc-950/80 p-4">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Suggested developers</h2>
                <Users className="size-4 text-zinc-500" />
              </div>
              <div className="space-y-3">
                {developers.map((developer) => (
                  <Link href={`/profile/${developer.username}`} key={developer.username} className="flex items-center justify-between rounded-md p-2 transition hover:bg-white/[0.04]">
                    <div className="min-w-0">
                      <div className="truncate text-sm font-semibold text-white">{developer.name}</div>
                      <div className="truncate text-xs text-zinc-500">{developer.role}</div>
                    </div>
                    <Badge variant="outline" className="border-emerald-400/20 text-emerald-200">Follow</Badge>
                  </Link>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-lg border border-white/10 bg-zinc-950/80 p-4">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-500">Notifications</h2>
                <Sparkles className="size-4 text-sky-300" />
              </div>
              <div className="space-y-3">
                {notifications.map((notification) => (
                  <div key={notification.text} className="rounded-md border border-white/5 bg-white/[0.03] p-3">
                    <p className="text-sm text-zinc-300">{notification.text}</p>
                    <p className="mt-1 text-xs text-zinc-600">{notification.time} ago</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-2">
                {communities.slice(0, 3).map((community) => (
                  <Link href="/communities" key={community.slug} className="flex items-center justify-between rounded-md px-2 py-2 text-sm transition hover:bg-white/[0.04]">
                    ...
                    <span className="text-xs text-zinc-500">{community.members}</span>
                  </Link>
                ))}
              </div>
            </div>
